...
운영체제는 컴퓨터 시스템을 편리하게 사용하기 위해 저장된 정보에 관한 논리적 정보를 제공, 
저장장치의 물리적 특성을 추상화하여 논리적 저장단위 파일을 정의
...
- 이름 : 사람의 편의를 위해 읽을 수 있는 형태로 유지된 유일한 정보
- 식별자 (identifier) : 파일 시스템 내에서 파일을 확인하는 고유 번호로, 사람이 식별할 수 없는 파일의 이름
- 유형 : 여러 유형을 제공하는 시스템을 위해 필요
- 위치 : 파일이 존재하는 장치 및 해당 장치 내 위치에 대한 포인터
- 크기 : 파일의 현재 크기(바이트, 워드 혹은 블록)와 최대 허용 가능한 크기
- 보호 : 접근 권한 정보
- 타임스탬프와 사용자 식별: 보호, 보안 및 사용자 감시를 위해 생성, 최근 변경, 최근 사용 등을 사용

## 파일 연산

운영체제는 파일 생성, 읽기, 쓰기, 위치 재설정, 삭제, 절단 등을 위한 시스템 콜 제공

- 파일 생성: 파일을 저장할 수 있도록 시스템 내 공간 찾음
- 파일 열기: 생성과 삭제를 제외한 모든 연산을 하기 전에는 반드시 파일 `open()`
- 파일 쓰기: 
  - 열린 파일 핸들과 파일에 기록될 정보를 명시한 시스템 콜 실행
  - 시스템은 파일 내의 다음 쓰기가 일어날 위치를 가리키는 쓰기 포인터를 유지하고 있어야한다.
  - 그 이유는 쓰기 연산이 발생할 때마다 쓰기 포인터가 갱신되어야 하기 때문이다.
- 파일 읽기
  - 파일 핸들과 파일이 읽혀야 할 메모리 내 블록의 위치를 기술하는 시스템 콜 사용
  - 시스템은 다음 순차적 읽기가 일어나 파일 안의 위치를 가리키는 읽기 포인터(read pointer)를 유지하고 있어야 한다.
  - 그 이유는 읽기 연산이 발생할 때마다 읽기 포인터가 갱신되어야 하기 때문이다.

프로세스는 일반적으로 파일 읽기나 파일 쓰기 중 하나를 하고있다.
대부분의 시스템은 하나의 현재 파일 위치 포인터를 가진다.
이렇게 읽기, 쓰기 연산 모두 해당 포인터를 사용함으로써 공간 절약 및 시스템의 복잡성 감소가 가능하다.
...
일부 환경에서는 open()연산 구현은 훨씬 복잡 → 프로세스 테이블, 시스템 테이블 사용
- 프로세스 테이블 : 접근 권한, 파일 포인터
- 시스템 테이블: 파일 위치, 크기, 날짜 등 (프로세스로부터 독립적)
...
파일들은 파일을 다루는 프로그램에 의해 인식 가능한 내부 구조를 일정 형태로 가진다.
- 일부 파일의 경우 운영체제가 인식할 수 있도록 미리 정해진 구조를 따라야 할 때도 있다.
- 여러 파일 구조를 지원하는 경우 → 운영체제 크기가 커지고 관리가 힘들어진다는 단점 존재한다.
- 모든 운영체제는 프로그램 로딩 및 실행할 수 있는 실행파일을 반드시 지원해야 한다.
...
### 직접 접근 상대 접근
- 어떤 블록 이라도 특별한 순차 없이 바로 빠르게 레코드 읽고 쓸 수 있다.
...
- 파일 연산이 블록 번호 파라미터를 포함할 수 있도록 수정되어야한다.

### 기타 접근 방법
- 직접 접근 파일이 있는 경우, 직접 접근 파일을 기반으로한 index을 사용하여 여러 다른 파일 접근을 제공한다.
...
- 파일이 많아지거나 다수의 사용자가 사용하는 시스템에서는 매우 비효율적이며 제약이 많다.
  - 한 명의 사용자만 사용한다고 해도 파일이 증가하는 경우 비효율적.
- 하나의 디렉터리에 모든 파일이 다 같이 존재하므로, 각 파일들은 유일한 이름을 가져야한다.
...
- UFD(User File Directory)
  - 사용자 본인만의 파일 디렉터리로, 오직 한 사용자만의 파일을 저장한다.
- MFD (Master File Directory)
  - 사용자의 이름이나 계정번호로 인덱싱 되어있고, 각 엔트리는 각 사용자의 UFD를 가리킨다.
...
- 각 사용자들에게 자신의 서브 디렉터리(subdirectory)를 원하는 만큼 만들 수 있게 한다.
- 최상위에 root 디렉터리 존재
- 시스템 내 모든 파일은 고유한 경로명을 가진다.
- 디렉터리 각 항목은 한 비트를 사용하여 해당 항목이 일반파일(0) 인지 디렉터리 파일(1)인지 구분
- 시스템에 따라 디렉터리 삭제 조건 다르다.
- 자신의 파일 뿐만 아니라 다른 사용자의 파일도 접근 할 수 있다.
...
- 공유 디렉터리 방법
  - 절대 경로명 혹은 상대경로명으로 구현된 링크 (다른 파일, 서브디렉터리를 가리키는 포인터)라는 새 디렉터리 항목을 만들 수 있음
  - 디렉터리들이 동일한 항목 , 내용을 복사해 가지고 있음
...
- 가비지 수집(garbage-collection)
  ...
  - 두번째 탐색에서 표시되지 않은 것들을 수집하고 사용가능한 공간 리스트에 추가한다.
...
- 보호는 가능한 파일 접근 유형을 제한함으로써 제한된 접근을 제공. 
- 여러 요인에 의해 접근 허용되거나 거부되는데 일부 유형의 연산들이 통제될 수 있다.
...
- 각 파일 접근시 암호 요구
- global table로 시스템 전체 file에 대한 권한은 table로 유지
- capability list로 메인에 대한 접근 권한 나열

## 메모리 사상 파일

memory mapping은 프로세스의 가상 메모리 주소 공간 중 일부를 관련된 파일에 할애한 뒤 가상 메모리 주소에 직접 접근 하는 것으로 파일 읽기 및 쓰기를 대신함
한다. (디스크 입출력을 메모리 참조 방식으로 대신한 것 )

### 기본 기법

- 메모리 매핑은 프로세스 페이지 중 일부를 디스크에 있는 파일 블록에 매핑함으로써 이루어진다.
- 메모리 매핑된 파일에 대한 쓰기는 반드시 보조저장장치의 파일에 즉각적으로 써지지 않음, 일반적으로 파일을 닫을 때만 메모리 변경사항에 따라 파일 업데이트
- 일부 운영체제는 특정 시스템 콜을 통해서만 메모리 매핑 파일 처리, 일반적으로는 표준파일 관련 시스템 콜 통해 파일에 접근하도록 한다.
...
- mapping될 파일에 대한 file mapping 생성
- 해당 파일의 프로세스 가상 주소 공간상의 view를 생성
- 다른 프로세스는 파일을 오픈한 뒤 해당 파일의 뷰를 가상 주소 공간상에 생성
